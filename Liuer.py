...
class RNNClassifier(torch.nn.Module):
    ...
    def forward(self, input, seq_lengths):
        ...
        # pack them up
        # make be sorted by descendent  打包变长序列
...
def make_tensors(names, countries):
    ...
    seq_lengths = torch.LongTensor(
        [sl[1] for sl in sequences_and_lengths])  # 单独把列表长度拿出来 （名字的长度）
    countries = countries.long()

    # make tensor of name,BatchSize x SeqLen   padding
    # 先做一个batchsize*max(seq_lengths)全0的张量
    ...
    seq_lengths, perm_idx = seq_lengths.sort(
        dim=0, descending=True)  # sort返回排完序的序列和对应的index
